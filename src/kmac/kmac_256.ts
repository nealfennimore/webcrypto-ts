/**
 * Code related to KMAC256. Requires Node.js 24.8.0 or higher.
 * @module
 */
 ...
 * @example
 * ```ts
 * const key = await KMAC256.generateKey({ length: 256 });
 * ```
 */
export const generateKey = async (
    ...
    const key = await KmacShared.generateKey(
        { ...algorithm, name: Alg.Code.KMAC256 },
        extractable,
        keyUsages
    );
...
/**
 * Import a KMAC256 key from the specified format
 * @example
 * ```ts
 * const key = await KMAC256.importKey("jwk", jwk);
 * ```
 * @example
 * ```ts
 * const key = await KMAC256.importKey("raw-secret", bytes);
 * ```
 */
...
/**
 * Export a KMAC256 key into the specified format
 * @example
 * ```ts
 * const jwk = await KMAC256.exportKey("jwk", key.self);
 * ```
 * @example
 * ```ts
 ...
 * const signature = await KMAC256.sign({ outputLength: 256 }, key.self, message);
 * ```
 * @example
 * ```ts
 * const message = new TextEncoder().encode("a message");
 ...
 * @example
 * ```ts
 * const isVerified = await KMAC256.verify({ outputLength: 256 }, key.self, signature, message);
 * ```
 * @example
 * ```ts
