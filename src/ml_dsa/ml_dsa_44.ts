/**
 * Code related to ML_DSA_44 (post-quantum signatures).
...
/**
 * Generate a new ML_DSA_44 keypair
 * @example
 * ```ts
 * const keyPair = await ML_DSA_44.generateKey();
 * ```
 * @example
 * ```ts
 * const keyPair = await ML_DSA_44.generateKey(false);
 * ```
 * @example
 * ```ts
 * const keyPair = await ML_DSA_44.generateKey(true, ['sign', 'verify']);
 * ```
 */
 ...
 * @example
 * ```ts
 * const keyPair = await ML_DSA_44.generateKeyPair();
 * ```
 */
export const generateKeyPair = generateKey;

/**
 * Import an ML_DSA_44 public or private key
 * @example
 * ```ts
 * const pubKey = await ML_DSA_44.importKey("raw-public", pubKeyBytes, true, ['verify']);
 * ```
 * @example
 * ```ts
 * const privKey = await ML_DSA_44.importKey("raw-seed", seedBytes, true, ['sign']);
 * ```
 */
...
/**
 * Export an ML_DSA_44 public or private key
 * @example
 * ```ts
 * const pubKeyBytes = await ML_DSA_44.exportKey("raw-public", keyPair.publicKey.self);
 * ```
 * @example
 * ```ts
 * const seedBytes = await ML_DSA_44.exportKey("raw-seed", keyPair.privateKey.self);
 * ```
 * @example
 * ```ts
 ...
 * const signature = await ML_DSA_44.sign(keyPair.privateKey.self, message);
 * ```
 * @example
 * ```ts
 * const message = new TextEncoder().encode("a message");
...
export async function sign(
    ...
    return await MlDsaShared.sign(
        { ...algorithm, name: Alg.Variant.ML_DSA_44 },
        key,
        data
    );
}
 ...
 * ```ts
 * const message = new TextEncoder().encode("a message");
 * const isVerified = await ML_DSA_44.verify(keyPair.publicKey.self, signature, message);
 * ```
 * @example
 * ```ts
 * const message = new TextEncoder().encode("a message");
