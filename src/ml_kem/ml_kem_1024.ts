/**
 * Code related to ML_KEM_1024 (post-quantum key encapsulation).
...
/**
 * Generate a new ML_KEM_1024 keypair
 * @example
 * ```ts
 * const keyPair = await ML_KEM_1024.generateKey();
 * ```
 * @example
 * ```ts
 * const keyPair = await ML_KEM_1024.generateKey(false);
 * ```
 * @example
 * ```ts
 * const keyPair = await ML_KEM_1024.generateKey(true, ['encapsulateBits', 'decapsulateBits']);
 * ```
 */
 ...
 * @example
 * ```ts
 * const keyPair = await ML_KEM_1024.generateKeyPair();
 * ```
 */
export const generateKeyPair = generateKey;

/**
 * Import an ML_KEM_1024 public or private key. Note that a public key
 * must be imported with encapsulation usages, and a private key with
 * decapsulation usages.
 * @example
 * ```ts
 * const pubKey = await ML_KEM_1024.importKey("raw-public", pubKeyBytes, true, ['encapsulateKey', 'encapsulateBits']);
 * ```
 * @example
 * ```ts
 * const privKey = await ML_KEM_1024.importKey("raw-seed", seedBytes, true, ['decapsulateKey', 'decapsulateBits']);
 * ```
 */
...
/**
 * Export an ML_KEM_1024 public or private key
 * @example
 * ```ts
 * const pubKeyBytes = await ML_KEM_1024.exportKey("raw-public", keyPair.publicKey.self);
 * ```
 * @example
 * ```ts
 * const seedBytes = await ML_KEM_1024.exportKey("raw-seed", keyPair.privateKey.self);
 * ```
 * @example
 * ```ts
 ...
 * @example
 * ```ts
 * const { sharedKey, ciphertext } = await ML_KEM_1024.encapsulateBits(keyPair.publicKey.self);
 * ```
 * @example
 * ```ts
 ...
 * @example
 * ```ts
 * const { sharedKey, ciphertext } = await ML_KEM_1024.encapsulateKey(
 ...
 * @example
 * ```ts
 * const sharedKey = await ML_KEM_1024.decapsulateBits(keyPair.privateKey.self, ciphertext);
 * ```
 * @example
 * ```ts
 ...
 * @example
 * ```ts
 * const sharedKey = await ML_KEM_1024.decapsulateKey(
