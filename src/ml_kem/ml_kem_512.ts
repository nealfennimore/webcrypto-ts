/**
 * Code related to ML_KEM_512 (post-quantum key encapsulation).
...
/**
 * Generate a new ML_KEM_512 keypair
 * @example
 * ```ts
 * const keyPair = await ML_KEM_512.generateKey();
 * ```
 * @example
 * ```ts
 * const keyPair = await ML_KEM_512.generateKey(false);
 * ```
 * @example
 * ```ts
 * const keyPair = await ML_KEM_512.generateKey(true, ['encapsulateBits', 'decapsulateBits']);
 * ```
 */
 ...
 * @example
 * ```ts
 * const keyPair = await ML_KEM_512.generateKeyPair();
 * ```
 */
export const generateKeyPair = generateKey;

/**
 * Import an ML_KEM_512 public or private key. Note that a public key
 * must be imported with encapsulation usages, and a private key with
 * decapsulation usages.
 * @example
 * ```ts
 * const pubKey = await ML_KEM_512.importKey("raw-public", pubKeyBytes, true, ['encapsulateKey', 'encapsulateBits']);
 * ```
 * @example
 * ```ts
 * const privKey = await ML_KEM_512.importKey("raw-seed", seedBytes, true, ['decapsulateKey', 'decapsulateBits']);
 * ```
 */
...
/**
 * Export an ML_KEM_512 public or private key
 * @example
 * ```ts
 * const pubKeyBytes = await ML_KEM_512.exportKey("raw-public", keyPair.publicKey.self);
 * ```
 * @example
 * ```ts
 * const seedBytes = await ML_KEM_512.exportKey("raw-seed", keyPair.privateKey.self);
 * ```
 * @example
 * ```ts
 ...
 * @example
 * ```ts
 * const { sharedKey, ciphertext } = await ML_KEM_512.encapsulateBits(keyPair.publicKey.self);
 * ```
 * @example
 * ```ts
 ...
 * @example
 * ```ts
 * const { sharedKey, ciphertext } = await ML_KEM_512.encapsulateKey(
 ...
 * @example
 * ```ts
 * const sharedKey = await ML_KEM_512.decapsulateBits(keyPair.privateKey.self, ciphertext);
 * ```
 * @example
 * ```ts
 ...
 * @example
 * ```ts
 * const sharedKey = await ML_KEM_512.decapsulateKey(
